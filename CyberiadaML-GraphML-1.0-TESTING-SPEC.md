...
- `CGML-6.3-4` MUST: transitions outgoing from the same `source` node must have **at most one** **else** trigger.[R/W/X]
...
- `CGML-6.5-2` Region marker: `dRegion` key on the nested graph (empty value). [R/W]
- `CGML-6.5-3` MAY: region/subgraph id = parent node id + separator (`:`). [W]
- `CGML-6.5-4` MUST: transitions of child elements are placed at the end of the **top-level** state-machine graph, not inside region subgraphs. [R/W/X]
...
- `CGML-6.6-2` MAY: `dData` — comment body text; `dName` — comment title; `dGeometry` (rectangle). [R/W]
- `CGML-6.6-3` SHOULD: it is recommended to preserve a formal comments body verbatim. [R]

**6.7 Comment-subject links** (`CGML-6.7-*`)
- `CGML-6.7-1` MUST: link = `edge` from comment node to subject node with mandatory first key `dPivot` naming the commented aspect (e.g. `dName`, `dData`). [R/W/X]
...
- `CGML-6.8-4` MUST: if the string is not empty the event description begins with the event name — a string conforming to the syntax of the target platform language; the name may be absent both in a transition label (completion transition) and in a node block. [R/W]
...
- `CGML-7.2-1-*` Geometry carriers (Table 4) [R/W]:
  ...
  - `CGML-7.2-1-5` transition — **no geometry in base format**;
  - `CGML-7.2-1-6` transition label — `dLabelGeometry` point or rect on the edge.
- `CGML-7.2-2` MUST: `point` carries `x`,`y` = element **center**; `rect` carries `x`,`y` = **top-left corner** + `width`,`height`. Coordinates are real numbers; SVG-like syntax `<point x= y=/>`, `<rect x= y= width= height=/>` nested inside the `data` tag. [R/W/X]
- `CGML-7.2-3` MUST: absence of `dGeometry` on an element = element is not visualized; a non-visualized composite state hides all its children. [R]
...
- `CGML-9.1-1` MUST: additionally (Table 6) transitions get geometry — `dGeometry` on edges (polyline/points), `dSourcePoint`/`dTargetPoint` keys for edge endpoint attachment; label geometry may be a rect. [R/W]
...
Here is the complete key inventory with element bindings. A document redefining a standard key differently, or using an undeclared key without declaration, violates 5.5. Reproduced for reference:

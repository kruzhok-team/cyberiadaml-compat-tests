...
`docs/PNST_1044-2025.md` is a Markdown mirror of the standard (with figures and
table images under `docs/images/`), generated by `tools/pnst2md.py` from the
PDF (requires `pypdf`, `pymupdf`, `pillow`) and reviewed by hand — the
reference for formatting questions that plain PDF text extraction distorts
(blank lines, tables, listings).
